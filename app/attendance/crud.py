...
class AttendanceCrud:
    ...
    def get_skipped_user_attendance(self, user_id: UUID):
        all_attendance = self.get_user_attendance(user_id=user_id)
        skipped_ones = []
        for attendance in all_attendance:
            if not attendance.attended:
                skipped_ones.append(attendance)
        return skipped_ones

    def get_promised_user_attendance(self, user_id: UUID):
        all_attendance = self.get_user_attendance(user_id=user_id)
        promised_ones = []
        for attendance in all_attendance:
            if attendance.promised:
                promised_ones.append(attendance)
        return promised_ones
